...
namespace RayTracer
{
		...
		: TriangleMesh(center)
	{
		 ...
		 * I built a physical model of an icosahedron and labeled the vertices using the point indices above.
		 * I then labeled the faces with lowercase letters as show in the comments for each AddTriangle call below.
		 * Each triplet of points in each AddTriangle call is arranged in counterclockwise order as seen from outside the icosahedron,
		 * so that vector cross products will work correctly to calculate the surface normal unit vectors for each face.
         */
