...
namespace RayTracer
{
		...
		std::vector<Intersection>& intersectionList) const
	{
		...
		{
            /**
			 * There are two intersection solutions, one involving +sqrt(radicand), the other -sqrt(radicand).
			 * Check both because there are weird special cases, like the camera being inside the sphere, or the sphere being behind the camera (invisible).
			 */
			...
			for(int i = 0; i < 2; ++i)
			{
				if(u[i] > EPSILON)
				{
					...
					// The normal vector to the surface of a sphere is outward from the center.
